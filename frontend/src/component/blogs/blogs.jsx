    ...
    {blogs.map(item => (
      <div key={item.title} className="md:px-2 md:py-6 max-md:p-2 w-full md:border-b border-gray-300">
        ...
        <div className="relative md:h-[400px] max-h-[60vh] w-full flex flex-wrap items-center justify-center md:p-2 gap-2">
          {item?.image.length !== 0 ? (
            item.image.length === 1 ? (
              <img src={`${link}/api/blog-images/${item.image[0]}`} 
                   alt={item.image[0]} 
                   className="w-full md:h-[400px] h-[210px] object-cover rounded-lg"/>
            ) : item.image.length === 2 ? (
              <div className="flex w-full gap-2">
                {item.image.map(img => (
                  <img src={`${link}/api/blog-images/${img}`} 
                       alt={img} 
                       className="w-1/2 md:h-[400px] h-[210px] object-cover rounded-lg"/>
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-2 w-full">
                ...
                <img src={`${link}/api/blog-images/${item.image[2]}`} 
                     alt={item.image[2]} 
